...
public sealed class SessionSupervisor : BackgroundService
{
    ...
    {
        ActivityLog.Write("service_started", $"checking every {PollInterval.TotalSeconds}s, target={AgentExePath}");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            catch (Exception ex)
            {
                ...
                ActivityLog.Write("session_agent_relaunch_failed", ex.Message);
            }
    ...
    private void SuperviseSessions()
    {
        ...
        foreach (var sessionId in activeSessionIds)
        {
            ...
            {
                ActivityLog.Write("session_agent_relaunched", $"session_id={sessionId}; path={AgentExePath}");
            }
            else
            {
                ActivityLog.Write("session_agent_relaunch_failed", $"session_id={sessionId}; error={error}");
            }
        }
    }
}
