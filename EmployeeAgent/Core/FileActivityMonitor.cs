...
public sealed class FileActivityMonitor : IDisposable
{
    ...
    private void StartFileOpenTracing()
    {
        ...
        {
            _logger.Log("file_open_tracking_failed", "must run elevated (Administrator) to trace file-open events");
            return;
        }
        ...
        try
        {
            ...
            _etwSession.Source.Kernel.FileIOCreate += data =>
            {
                ...
                _logger.Log("file_opened", $"process={processName}; path={data.FileName}");
            };
        ...
        {
            _logger.Log("file_open_tracking_failed", ex.Message);
        }
    }
