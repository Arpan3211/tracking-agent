...
public sealed class AgentContext : ApplicationContext
{
    ...
    private static readonly TimeSpan SyncTickInterval = TimeSpan.FromSeconds(4);
    ...
    public AgentContext()
    {
        ...
        _logger.Log("agent_started", new() { ["user"] = Environment.UserName, ["machine"] = Environment.MachineName });

        // SystemEvents.SessionSwitch only delivers FUTURE transitions - by
        // the time this process is launched (up to ~30s after the actual
        // logon, via the anti-tamper service's SessionSupervisor), the real
        // SessionLogon has already happened and this subscription can never
        // see it. Backfill the real logon time from the OS instead of
        // relying on catching it live - this is what makes session
        // start times (and therefore session duration) accurate rather
        // than silently missing for the common case. Falls back to "now"
        // only if the WTS query itself fails.
        _logger.Log(
            "login",
            SessionInfo.GetSessionLogonTimeUtc() ?? DateTime.UtcNow,
        ...
        // Ticks every 4s and sends immediately whenever anything is queued -
        // see SyncLoop.TickAsync.
    ...
    private void OnSessionSwitch(object sender, SessionSwitchEventArgs e)
    {
        switch (e.Reason)
        {
            case SessionSwitchReason.SessionLogon:
                _logger.Log("login", new() { ["username"] = Environment.UserName });
                break;
            case SessionSwitchReason.SessionLogoff:
                _logger.Log("logout", new() { ["username"] = Environment.UserName });
                break;
    ...
    private void CheckIdleState()
    {
        ...
        if (idleFor >= IdleThreshold && !_isCurrentlyIdle)
        {
            ...
            _logger.Log("idle_start", new() { ["idle_for_seconds"] = idleFor.TotalSeconds.ToString("F0") });
        }
    ...
    protected override void ExitThreadCore()
    {
        ...
        _logger.Dispose(); // drains any still-queued writes before closing the connection
        _hiddenForm.Dispose();
        base.ExitThreadCore();
    }
}
