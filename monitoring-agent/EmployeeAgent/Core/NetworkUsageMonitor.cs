...
public sealed class NetworkUsageMonitor : IDisposable
{
    ...
    public void Start()
    {
        ...
        {
            _logger.Log("network_monitor_failed", new() { ["message"] = "must run elevated (Administrator) to trace per-process network usage" });
            return;
        }
        ...
        {
            _logger.Log("network_monitor_failed", new() { ["message"] = ex.Message });
        }
    }
    ...
    public void Flush()
    {
        ...
        foreach (var (processId, counts) in snapshot)
        {
            ...
            _logger.Log("network_usage", new()
            {
                ["process"] = processName,
                ["bytes_sent"] = counts.sent.ToString(),
                ["bytes_received"] = counts.received.ToString(),
            });
        }
    }
