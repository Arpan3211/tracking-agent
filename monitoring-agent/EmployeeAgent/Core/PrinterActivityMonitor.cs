...
public sealed class PrinterActivityMonitor
{
    ...
    private readonly Dictionary<uint, Dictionary<string, string>> _jobDetailsById = new();
    ...
    public void Poll()
    {
        ...
        {
            _logger.Log("printer_monitor_failed", new() { ["message"] = ex.Message });
            return;
        }
        ...
        {
            var details = _jobDetailsById.GetValueOrDefault(jobId, new Dictionary<string, string> { ["job_id"] = jobId.ToString() });
